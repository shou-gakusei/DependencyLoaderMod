...
public static class DependencyLoadService
{
    ...
    // 日志实例由 DependencyLoaderModPlugin.OnLoadAsync 注入（4.1 已移除 ServiceLocator）
    internal static ISptLogger<DependencyLoaderModPlugin>? Logger { get; set; }
    ...
    public static void AddResolveDirectory(string directoryPath)
    {
        ...
        {
            Logger?.Warning($"[DependencyLoaderMod] Resolve directory does not exist, skipping: {directoryPath}");
            return;
        }
        ...
        {
            Logger?.Info($"[DependencyLoaderMod] Registered resolve directory: {normalizedPath}");
        }
    }
    ...
    public static void AddNativeResolve(string assemblyName, string nativeDllName, string nativeDllPath)
    {
        ...
        {
            Logger?.Warning($"[DependencyLoaderMod] Native DLL not found, skipping: {nativeDllPath}");
            return;
        }

        NativeResolveMap[assemblyName] = (nativeDllName, Path.GetFullPath(nativeDllPath));
        Logger?.Info($"[DependencyLoaderMod] Registered native resolve: [{assemblyName}] {nativeDllName} -> {nativeDllPath}");
    }
    ...
    public static async Task<bool> DownloadPackageAsync(string packageName, string version, string targetDir)
    {
        try
        {
            ...
            try
            {
                ...
                Logger?.Info($"[DependencyLoaderMod] Downloading {packageName} {version} from NuGet...");
                ...
                Logger?.Info($"[DependencyLoaderMod] Downloaded {packageName} {version}. Extracting...");
                ...
                if (Directory.Exists(libDir))
                {
                    ...
                    foreach (var frameworkDir in Directory.GetDirectories(libDir))
                    {
                        ...
                        if (frameworkName is "net10.0" or "net9.0" or "netstandard2.0")
                        {
                            foreach (var dllFile in Directory.GetFiles(frameworkDir, "*.dll"))
                            {
                                ...
                                copiedCount++;
                                Logger?.Info($"[DependencyLoaderMod] Copied dependency: {Path.GetFileName(dllFile)}");
                            }
                        }
                    }
                    ...
                    if (copiedCount == 0)
                    {
                        Logger?.Warning($"[DependencyLoaderMod] No DLLs found in lib/net10.0/, lib/net9.0/ or lib/netstandard2.0/ for {packageName} {version}.");
                    }
                }
                else
                {
                    Logger?.Warning($"[DependencyLoaderMod] No lib/ directory found in extracted package {packageName} {version}.");
                }

                Logger?.Success($"[DependencyLoaderMod] Successfully processed NuGet package: {packageName} {version}");
                return true;
            }
            finally
            {
                ...
                {
                    Logger?.Warning($"[DependencyLoaderMod] Failed to clean up temp directory {tempDir}: {cleanupEx.Message}");
                }
            }
        }
        catch (HttpRequestException httpEx)
        {
            Logger?.Warning($"[DependencyLoaderMod] HTTP error downloading {packageName} {version}: {httpEx.Message}");
        }
        catch (TaskCanceledException timeoutEx)
        {
            Logger?.Warning($"[DependencyLoaderMod] Download timeout for {packageName} {version}: {timeoutEx.Message}");
        }
        catch (Exception ex)
        {
            Logger?.Warning($"[DependencyLoaderMod] Failed to download/extract {packageName} {version}: {ex.Message}");
        }
        ...
        try
        {
            ...
            try
            {
                ...
                if (nugetSourceUrl.Contains("/v3/index.json", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        Logger?.Info($"[DependencyLoaderMod] Downloading {packageName} {version} via NuGet.Protocol...");
                    }
                    ...
                    if (!downloaded)
                    {
                        ...
                        else Logger?.Warning(fallbackMsg);
                    }
                }
                ...
                if (!downloaded)
                {
                    ...
                    {
                        Logger?.Info($"[DependencyLoaderMod] Downloading {packageName} {version}...");
                    }
                    ...
                    {
                        Logger?.Info($"[DependencyLoaderMod] Downloaded {packageName} {version} ({bytesReadSoFar} bytes). Extracting...");
                    }
                ...
                if (Directory.Exists(libDir))
                {
                    ...
                    foreach (var frameworkDir in Directory.GetDirectories(libDir))
                    {
                        ...
                        // 接受常见的 .NET 目标框架
                        if (frameworkName is "net10.0" or "net9.0" or "net8.0" or "net7.0" or "netstandard2.0" or "net6.0" or "net5.0" or "netcoreapp3.1" or "net48" or "net472" or "net462")
                        {
                            foreach (var dllFile in Directory.GetFiles(frameworkDir, "*.dll"))
                            {
                                var destFile = Path.Combine(targetDir, Path.GetFileName(dllFile));
                    ...
                    foreach (var ridDir in Directory.GetDirectories(runtimesDir))
                    {
                        ...
                        if (Directory.Exists(nativeDir))
                        {
                            ...
                            foreach (var nativeFile in Directory.GetFiles(nativeDir))
                            {
                                ...
                                {
                                    Logger?.Info($"[DependencyLoaderMod] Copied native dependency: runtimes/{ridName}/native/{Path.GetFileName(nativeFile)}");
                                }
                            }
                        }
                    }
